...
**Bonus verification, unplanned:** while testing this fix, the owner's overnight Daphne-native game transfer (`ace`, i.e. Space Ace) was already on the device. The scanner correctly detected it as `DAPHNE_NATIVE` - verifying #28's previously-untested Daphne-native detection path against real data for the first time. It doesn't fully launch yet (`ROM sa_a3_u1.bin couldn't be found` - an incomplete ROM zip on the device, not an app bug), but the detection and launch-arg construction both worked correctly up to that point.
