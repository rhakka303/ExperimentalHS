...
**Bug 4 - `-homedir`/`-datadir` must be the *parent* of the picked/scanned game folder, not the folder itself.** Every Singe/LaserForge game's own `.singe` script follows a standard convention (confirmed by reading a real unzipped script, `AlteredCarbonResleeved.singe`): `BASEDIR = "singe"`, `MYDIR = BASEDIR.."/"..gamename`, `dofile(BASEDIR.."/Framework/globals.singe")` - meaning the script itself expects a `singe/` subfolder under the *real* homedir, matching hypseus's own upstream doc example exactly. `LaunchArgs.kt` passes `File(homeDir).parent` (not `homeDir`) as `-homedir`/`-datadir` for this reason. Note: this fix is real and necessary, but on its own did **not** resolve the crash described below - `luaL_openlibs()` runs before the game script is ever read, so a failed `dofile()` downstream couldn't have caused it. Recorded here for correctness, not as the fix for Bug 5.

**Bug 5 (the big one) - a native SIGABRT inside hypseus-singe's own forked Lua 5.1 garbage collector, triggered by weak-table traversal.** `FORTIFY: strchr: prevented read past end of buffer`, precisely inside `traversetable()` (`lgc.c:167`, `strchr(svalue(mode), 'k')` on a table's `__mode` weak-table metafield) - confirmed via exact function-symbol boundaries from `llvm-nm -S` against the unstripped build artifact (`android/app/build/intermediates/cmake/debug/obj/arm64-v8a/libmain.so`), not just DWARF line-table guessing, which was misleading for several of the inner frames. Reached via `luaC_step → singlestep → propagatemark → traversetable`, itself triggered mid-allocation during `lua_pushcclosure()` inside library/function registration. hypseus-singe's own `linit.c` registers **16 libraries**, not stock Lua's 6 (`lfs`, `bit`, `cjson`, `md5`, `socket`, `lpeg`, `luars232`, `random` on top of the standard set) - several of these C extension libraries are known to use weak tables. `-fno-strict-aliasing` was tried on the whole `singeproxy` CMake target (a well-known real Lua 5.1 build requirement with existing ARM-specific-flag precedent already in this exact file, `-Wno-psabi`) - did **not** fix it, ruling out simple aliasing/type-punning UB. The deeper "why is this string corrupted" question was never fully answered. This is almost certainly a genuine, previously-undiscovered arm64 portability bug in this Lua fork - as far as could be determined, **this is the first-ever arm64 Android build of hypseus-singe**.

**The workaround that got a real game booting: hold Lua's GC off across the entire startup/script-load phase.** `singeproxy.cpp`'s `sep_startup()`: `lua_gc(g_se_lua_context, LUA_GCSTOP, 0)` right after `lua_open()`, `lua_gc(g_se_lua_context, LUA_GCRESTART, 0)` at the very end of the function (after the final `luaL_dofile()` that runs the actual game script). Confirmed via a deliberate diagnostic - stopping GC only around `luaL_openlibs()` *moved* the crash later (into `sep_startup()`'s own ~100+ custom `lua_register()` calls) rather than eliminating it, proving the GC-triggered-traversal mechanism rather than just correlating with it. **This is a workaround, not a real fix** - GC is fully disabled for the whole startup window (bounded memory growth, acceptable for a short burst) and, confirmed by later testing, the same crash can still recur *during real gameplay* once GC resumes after startup (it did, when opening an in-game menu). A proper fix needs either a live debugger attach to find what's actually corrupting the weak table's string, or much broader GC-pause tuning accepting the memory tradeoff. Tracked as a real follow-up, not closed out by this workaround.

**Result: a real Singe game boots and plays on the physical Retroid Pocket 5 for the first time.** Verified via hypseus's own log (`homedir/logs/hypseus.log`): clean video init (zero `pics/` errors), `SDL_gamepad_init` detecting "Retroid Pocket Controller", `.zip` ROM loading, config files copied out of the zip, correct viewport scaling (640×480 → 1600×900), a graceful fallback for a missing localized audio track, and real full-motion video rendering with the `pics/`-based overlay ("FREE PLAY" text) drawn correctly on screen (confirmed via device screenshot, not just log lines). Input verified end-to-end two ways: a simulated `adb shell input keyevent` correctly produced `SINGE: "User requested a quit."` in hypseus's own log (proof the full SDL-gamepad-to-Lua pipeline works), and the owner directly confirmed on the physical controller that directions and START work, reaching real gameplay past the start screen. `hypinput_gamepad.ini` behavior confirmed via source read (`input.cpp`'s `defaultConfig()` only writes if the file doesn't already exist - manual edits are permanent) and used to bind the left-stick click to the in-game service/options menu.

**Known remaining gap, not blocking further Phase D work:** Bug 5's underlying corruption is unresolved: the GC-stop workaround only covers startup, and the same crash reproduced later when the owner opened the in-game service menu during real play. This is accurately described as "boots and plays past the point that used to block it entirely," not "fully stable" - worth its own follow-up issue to properly root-cause rather than continuing to rely on disabling GC.
